...
function Kudu-ZipDeploy {
  ...
  $creds = Get-KuduCredentials -AppName $AppName
  $endpoint = "$($creds.url)/api/zipdeploy?deployer=script"
  $pair = "$($creds.user):$($creds.pass)"
  $bytes = [System.Text.Encoding]::ASCII.GetBytes($pair)
  $b64 = [Convert]::ToBase64String($bytes)
...
function Package-Server {
  ...
  Copy-Item (Join-Path $ServerDir 'dist') $ServerPkgDir -Recurse
...
'@
  ...
  if (-not $WhatIf) {
    ...
    } else { Write-Info '[Verify] Server package contains package.json, dist/index.js, startup.js' }
